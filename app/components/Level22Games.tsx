...
interface Level22GamesProps {
    levelId: string;
    onComplete: (data: any) => void;
    initialData?: Record<string, string>;
}

export default function Level22Games({ levelId, onComplete, initialData }: Level22GamesProps) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (!allAnswered) {
            setWarning("Silakan tuliskan tujuanmu!")
            return;
        }
        setSubmitting(true)
        onComplete(answers)
    ...
    // Stacked inputs on the bottom board area
    const getPositionClass = (id: string) => {
        switch (id) {
            case 'main_goal': return "top-[80%] left-[50%] -translate-x-1/2 -translate-y-1/2 w-[60%] h-[30%]"
            ...
            <div className="min-h-screen bg-[#e0f7fa] py-6 px-4 font-fredoka flex items-center justify-center" style={{ backgroundImage: 'radial-gradient(#b2ebf2 10%, transparent 10%)', backgroundSize: '20px 20px' }}>
                <div className="relative h-[85vh] aspect-[1131/1600] animate-[popIn_0.5s_cubic-bezier(0.175,0.885,0.32,1.275)] mx-auto">
                    ...
                    <div className="w-full h-full bg-white rounded-[30px] shadow-2xl overflow-hidden border-[8px] border-[#0097a7] relative">
                        <Image
                            src="/images/level22-bg.jpeg"
                            alt="Level 22 Background"
                            fill
                            ...
                            {questions.map((q) => (
                                ...
                                >
                                    <textarea
                                        ...
                                        className="w-full h-full bg-transparent border-none rounded-xl px-4 py-4 text-center font-bold text-[#3e2723] text-sm md:text-base focus:outline-none focus:ring-0 transition-all leading-relaxed resize-none flex items-center justify-center p-0"
                                        placeholder=""
                                    />
                                </div>
                            ))}
