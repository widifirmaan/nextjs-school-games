...
interface Level17GamesProps {
    levelId: string;
    onComplete: (data: any) => void;
    initialData?: Record<string, string>;
}

export default function Level17Games({ levelId, onComplete, initialData }: Level17GamesProps) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (!allAnswered) {
            setWarning("Silakan isi semua sticky notes!")
            return;
        }
        setSubmitting(true)
        onComplete(answers)
    ...
    // Positions for 4 sticky notes (Grid Layout Guess)
    const getPositionClass = (id: string) => {
        switch (id) {
            case 'sticky_1': return "top-[33%] left-[25%] -translate-x-1/2 -translate-y-1/2 w-[35%] h-[20%]" // Top Left
            case 'sticky_2': return "top-[40%] right-[25%] translate-x-1/2 -translate-y-1/2 w-[35%] h-[20%]" // Top Right
            case 'sticky_3': return "top-[68%] left-[25%] -translate-x-1/2 -translate-y-1/2 w-[35%] h-[20%]" // Bottom Left
            case 'sticky_4': return "top-[85%] right-[25%] translate-x-1/2 -translate-y-1/2 w-[35%] h-[20%]" // Bottom Right (Moved Down again)
            ...
            <div className="min-h-screen bg-[#fffde7] py-6 px-4 font-fredoka flex items-center justify-center" style={{ backgroundImage: 'radial-gradient(#fff9c4 10%, transparent 10%)', backgroundSize: '20px 20px' }}>
                <div className="relative h-[85vh] aspect-[1131/1600] animate-[popIn_0.5s_cubic-bezier(0.175,0.885,0.32,1.275)] mx-auto">
                    ...
                    <div className="absolute -top-4 left-1/2 transform -translate-x-1/2 z-20 w-3/4">
                        <div className="bg-[#fbc02d] text-[#3e2723] py-2 text-center rounded-full border-b-[6px] border-[#f9a825] shadow-lg">
                            <span className="font-black uppercase tracking-widest text-lg md:text-xl whitespace-nowrap px-4">
                                {levelConfig.title || "LEVEL 17"}
                            </span>
                        </div>
                    </div>
                    ...
                    <div className="w-full h-full bg-white rounded-[30px] shadow-2xl overflow-hidden border-[8px] border-[#fbc02d] relative">
                        <Image
                            src="/images/Level17-bg.jpeg"
                            alt="Level 17 Background"
                            fill
                            ...
                            {questions.map((q) => (
                                ...
                                >
                                    <textarea
                                        ...
                                        className="w-full h-full bg-transparent border-none rounded-xl px-4 py-3 text-center font-bold text-[#3e2723] text-sm md:text-base focus:outline-none focus:ring-0 transition-all placeholder-[#3e2723]/50 leading-relaxed resize-none flex items-center justify-center p-0"
                                        placeholder="..."
                                    />
                                </div>
                            ))}
